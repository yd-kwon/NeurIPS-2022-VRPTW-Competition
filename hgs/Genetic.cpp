...
void Genetic::run(int maxIterNonProd, double timeLimit)
{
	...
	for (int nbIter = 0; nbIterNonProd <= maxIterNonProd && !params->isTimeLimitExceeded(); nbIter++)
	{
		...
		Individual* offspring = bestOfSREXAndOXCrossovers(population->getNonIdenticalParentsBinaryTournament(nbIterNonProd));
		...
		{
			nbIterNonProd = 1;
			population->savePopulations(params->getTimeElapsedSeconds());
		}
		else nbIterNonProd++;
		...
		if (timeLimit < 3600*23 && nbIterNonProd == maxIterNonProd && params->config.doRepeatUntilTimeLimit)
		{
			params->config.nbGranular += 5;
			params->SetCorrelatedVertices();

			params->config.circleSectorOverlapToleranceDegrees += 5;
			
			double remainTime = params->getRemainTime();
			if ( remainTime > 5 ) population->restart(remainTime);
			nbIterNonProd = 1;
		}
