...
Params::Params(const CommandLine& cl)
{
	...
	// Configurations for NeurIPS Run
	std::cout << "----- NEURIPS RUN: " << config.isNeurIPSRun << std::endl;
	if (config.isNeurIPSRun)
	{
		if ( config.verbose == true )
			std::cout << "NeurIPS RUN for instance name " << instanceName << ", writing solution to " << config.pathSolution << std::endl;

		nbParentTournament = 4;
		bAdaptiveParentTounamentA = true;
		bAdaptiveParentTounamentB = true;

		// population size
		if (nbClients < 300) 
		{
			config.minimumPopulationSize = 40;
			config.generationSize = 45;
		}
		else
		{
			config.minimumPopulationSize = 50;
			config.generationSize = 40;
		}		

		// nbIter
		if ( config.bAdaptiveNbIter == true )
		{
			if ( nbClients >= 500 ) config.nbIter = config.nbIter*2;
			else if ( nbClients >= 300 ) config.nbIter = config.nbIter*3/2;
		}
...
double Params::getRemainTime(){
	return config.timeLimit - getTimeElapsedSeconds();	
}
