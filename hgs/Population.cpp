...
void Population::generatePopulation()
{
	...
	double numberOfPresol = params->config.numberOfPresol;
	...
	int nofPresolIndivisualsToGenerate = numberOfPresol;

	if ( nofPresolIndivisualsToGenerate > 0 && params->config.pathPresol.size() == 0 ) 
	{
		std::cout << "Path of the presolution is not defined, use -pathPresol" << std::endl;
		nofPresolIndivisualsToGenerate = 0;
	}
	...
	// Generate some individuals using a Presolution
	for (int i = 0; i < nofPresolIndivisualsToGenerate; i++)
	{
		if (params->isTimeLimitExceeded())
		{
			std::cout << "Time limit during generation of initial population" << std::endl;
		...
		Individual randomIndiv(params, false);
		randomIndiv.loadPreSolution(i);
		...
		if ( params->config.verbose == true )
		{
			if (((i+1)%1000)==0)
			{
				std::cout << "Generated " << i+1 << " individuals from Pre-solutions" << std::endl;
				printState(-1, -1);	
			}
		}
	}
	...
	if ( params->config.verbose == true )
	{	
		std::cout << "Generated " << nofPresolIndivisualsToGenerate << " individuals from Pre-solutions" << std::endl;
		printState(-1, -1);	
	}
}
	...
	else
	{
		...
		for (int i = 0; i < static_cast<int>(pop.size()); i++)
		{
			...
			else if (params->config.diversityWeight > 0)
			{
				...
				if (pop[ranking[i].second]->age > params->config.maxAge) 
					pop[ranking[i].second]->biasedFitness += (pop[ranking[i].second]->age - params->config.maxAge);
			}
			else
			{
...
void Population::removeWorstBiasedFitness(SubPopulation& pop)
{
	...
	for (int i = 0; i < static_cast<int>(pop.size()); i++)
	{
		...
		if ((isClone && !isWorstIndividualClone) || (!isClone && !isWorstIndividualClone && pop[i]->biasedFitness > worstIndividualBiasedFitness) || (isClone && isWorstIndividualClone && pop[i]->age < worstIndividualAge))
		{
			worstIndividualBiasedFitness = pop[i]->biasedFitness;
			worstIndividualAge = pop[i]->age;
...
void Population::restart(double dTimeRemain)
{
	std::cout << "----- RESET: CREATING A NEW POPULATION -----    dTimeRemain: " << dTimeRemain << std::endl;
	...
	params->config.fractionGeneratedNearest = 0.25;
	params->config.fractionGeneratedFurthest = 0.25;
	params->config.fractionGeneratedSweep = 0.25;
	params->config.fractionGeneratedRandomly = 0.25;
	
	// Generate a new initial population
	restorePopulations(dTimeRemain);
	generatePopulation();
...
Individual* Population::getBinaryTournament(int nbParentTournament)
{
	...
	for (int i = 0 ; i < nbParentTournament; i++)
	{
...
std::pair<Individual*, Individual*> Population::getNonIdenticalParentsBinaryTournament(int nbIterNonProd)
{
	...
	int nbNumPTA = params->nbParentTournament;
	int nbNumPTB = params->nbParentTournament;

	if ( params->bAdaptiveParentTounamentA )
	{
		int maxIter = params->config.nbIter;
		if ( nbIterNonProd*2 > maxIter )      nbNumPTA = std::max(1, nbNumPTA/2);
	}

	if ( params->bAdaptiveParentTounamentB )
	{
		int maxIter = params->config.nbIter;
		if ( nbIterNonProd*4/3 > maxIter )	  nbNumPTB = std::max(1, nbNumPTB/4);
		else if ( nbIterNonProd*2 > maxIter ) nbNumPTB = std::max(1, nbNumPTB/2);
		else if ( nbIterNonProd*4 > maxIter ) nbNumPTB = std::max(1, nbNumPTB*3/4);		
	}
	
	// Pick two individual using a binary tournament
	Individual* parentA = getBinaryTournament(nbNumPTA);
	Individual* parentB = getBinaryTournament(nbNumPTB);
	int num_tries = 1;
	...
	{
		parentB = getBinaryTournament(nbNumPTB);
		num_tries++;
	}
...
{
	if ( params->config.verbose == false ) return;
	...
	{
		std::printf(" | Feas %zu %.2f %.2f %.2f", feasibleSubpopulation.size(), getBestFeasible()->myCostSol.penalizedCost, getAverageCost(feasibleSubpopulation), getAverageAge(feasibleSubpopulation));
	}
	else
	{
	...
	{
		std::printf(" | Inf %zu %.2f %.2f %.2f", infeasibleSubpopulation.size(), getBestInfeasible()->myCostSol.penalizedCost, getAverageCost(infeasibleSubpopulation), getAverageAge(infeasibleSubpopulation));
	}
	else
	{
...
double Population::getAverageAge (const SubPopulation& pop)
{
	// The average cost of the population: The average of the penalizedCost over the best "mu" individuals of the population
	double average = 0.;
...
void Population::savePopulations(double dCurrTime)
{
	SubPopulation fpop;
	SubPopulation ifpop;
	
	for (int i = 0; i < params->config.minimumPopulationSize && i < static_cast<int>(feasibleSubpopulation.size()); i++)
	{
		Individual* indiv = new Individual(*feasibleSubpopulation[i]);
		fpop.push_back(indiv);
	}	

	for (int i = 0; i < params->config.minimumPopulationSize && i < static_cast<int>(infeasibleSubpopulation.size()); i++)
	{
		Individual* indiv = new Individual(*infeasibleSubpopulation[i]);
		ifpop.push_back(indiv);
	}	


	savedTimes.push_back(dCurrTime);
	savedFeasiblePop.push_back(fpop);
	savedInfeasiblePop.push_back(ifpop);

	if ( params->config.verbose ) 
	{
		std::printf("[savePopulations] curr time: %.2f, saved size: %d\n", dCurrTime, static_cast<int>(savedTimes.size()));
	}
...
void Population::restorePopulations_idx(int restore_idx)
{
	SubPopulation fpop = savedFeasiblePop[restore_idx];
	SubPopulation ifpop = savedInfeasiblePop[restore_idx];

	for (int i = 0; i < static_cast<int>(fpop.size()); i++) 
	{
		if (params->isTimeLimitExceeded()) return;
		addIndividual(fpop[i], true);
	}
	
	for (int i = 0; i < static_cast<int>(ifpop.size()); i++) 
	{
		if (params->isTimeLimitExceeded()) return;
		addIndividual(ifpop[i], true);		
	}
}

bool Population::restorePopulations(double dTimeOffset)
{
	if ( params->config.verbose ) std::printf("[restorePopulations] dTimeOffset: %.2f, saved size: %d\n", dTimeOffset, static_cast<int>(savedTimes.size()));

	int saved_num = static_cast<int>(savedTimes.size());
	if ( saved_num == 0 ) return false;

	double last_time_updated = savedTimes[saved_num-1];
	double target_time = last_time_updated - dTimeOffset;

	// find restore index
	int restore_idx = saved_num-1;
	while(restore_idx >= 0 && savedTimes[restore_idx] > target_time) restore_idx--;		
	if ( params->config.verbose ) 
	{
		std::printf("[restorePopulations] dTimeOffset: %.2f, last_time_updated: %.2f, target_time: %.2f, restore_idx: %d\n", dTimeOffset, last_time_updated, target_time, restore_idx);
	}
	if ( restore_idx < 0 ) return false;


	// restore	
	restorePopulations_idx(restore_idx);

	clearSavedPopulations(restore_idx+1);
	adjustSavedTimes(params->getTimeElapsedSeconds()-savedTimes[restore_idx]);

	if ( params->config.verbose ) std::printf("[restorePopulations] succeeded\n");

	return true;
}


void Population::adjustSavedTimes(double dTimeDiff)
{	
	for (int i = 0; i < static_cast<int>(savedTimes.size()); i++)
	{
		savedTimes[i] = savedTimes[i] + dTimeDiff;
	}
}

void Population::clearSavedPopulations(int start)
{
	if ( params->config.verbose ) std::printf("[clearSavedPopulations]\n");

	for ( int p = start; p < static_cast<int>(savedFeasiblePop.size()); p++)
	{
		SubPopulation pop = savedFeasiblePop[p];
		for (int i = 0; i < static_cast<int>(pop.size()); i++)
		{
			delete pop[i];
		}
	}
	savedFeasiblePop.resize(start);

	for ( int p = start; p < static_cast<int>(savedInfeasiblePop.size()); p++)
	{
		SubPopulation pop = savedInfeasiblePop[p];
		for (int i = 0; i < static_cast<int>(pop.size()); i++)
		{
			delete pop[i];
		}
	}	
	savedInfeasiblePop.resize(start);

	savedTimes.resize(start);
}
