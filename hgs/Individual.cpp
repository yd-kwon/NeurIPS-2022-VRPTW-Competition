...
bool Individual::readPresolutionFile(std::string fileName)
{
	...
	if (inputFile.is_open())
	{
		...
		for (int r = 0; inputString == "Route"; r++)
		{
			int route_index = r < params->nbVehicles ? r : params->nbVehicles-1; 
			...
			while (ss >> inputCustomer)
			{
				chromR[route_index].push_back(inputCustomer);
...
bool Individual::loadPreSolution(int index)
{
	char buf[2048];
	snprintf( buf, 2048, "/%06d.sol", index );
	std::string filename( buf );
	std::string preSolutionPath = params->config.pathPresol + filename; 
	
	bool ret = readPresolutionFile(preSolutionPath);
	if (ret==false)
	{		
		std::cout << "READ pre-solution : " << preSolutionPath << ", Fail" << std::endl;
		shuffleChromT();
		return false;
	}
	
	//std::cout << "Debug, READ pre-solution : " << preSolutionPath << ", Success " << std::endl;
