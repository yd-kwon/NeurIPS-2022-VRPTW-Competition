...
void LocalSearch::updateRouteData(Route* myRoute)
{
	...
	while (!mynode->isDepot || firstIt)
	{
		...
		myHasMustDispatch |= static_cast<bool>(params->cli[mynode->cour].mustDispatch);
	...
	myRoute->hasMustDispatch = myHasMustDispatch;
...
CostSol LocalSearch::getCostSol(bool usePenaltiesLS)
{
	...
	for (int r = 0; r < params->nbVehicles; r++)
	{
		...
		int distance = myRoute->duration;
		double priority = 0;
		int nbCustom = 0;
		int capacityExcess = std::max(0, myRoute->load - params->vehicleCapacity);
		int timeWarp = myRoute->twData.timeWarp;
		int load = 0;
		int waitTime = 0; // TODO
		int nbRoutes = 0; // TODO
		int nbRoutesMustDispatch = static_cast<int>(myRoute->hasMustDispatch);

		myCostSol.update(distance, priority, nbCustom, capacityExcess, load, timeWarp, waitTime, nbRoutes, nbRoutesMustDispatch,
		                 params->vehicleCapacity, params->config.capacityIdleThreshold, myRoute->hasMustDispatch);
	}
	...
	if (usePenaltiesLS)
	{
	    double penaltyCapacityIdle = penaltyCapacityLS * params->config.penaltyCapacityReduction;
		myCostSol.penalizedCost = myCostSol.calculatePenalizedCost(params->nbClients, params->config.dispatchNumLim, params->config.penaltyNumCustom, params->config.penaltyTheRatio, penaltyCapacityLS, penaltyCapacityIdle, penaltyTimeWarpLS, params->penaltyWaitTime);
	}
	else
	{
