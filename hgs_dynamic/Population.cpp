...
void Population::managePenalties()
{
	...
	for (int i = 0; i < static_cast<int>(infeasibleSubpopulation.size()); i++)
	{
		...
		infeasibleSubpopulation[i]->myCostSol.penalizedCost = infeasibleSubpopulation[i]->myCostSol.calculatePenalizedCost(params->nbClients, params->config.dispatchNumLim, params->config.penaltyNumCustom, params->config.penaltyTheRatio, params->penaltyCapacity, penaltyCapacityIdle, params->penaltyTimeWarp, 0.);
	}
...
std::pair<Individual*, Individual*> Population::getNonIdenticalParentsBinaryTournament(int nbIterNonProd)
{
	...
	if ( params->bAdaptiveParentTounamentA )
	{
		...
		if ( nbIterNonProd*2 > maxIter ) nbNumPTA = std::max(1, nbNumPTA*2/3);
	}
	...
	if ( params->bAdaptiveParentTounamentB )
	{
		...
		if ( nbIterNonProd*4/3 > maxIter )	  nbNumPTB = std::max(1, nbNumPTB/3);
		else if ( nbIterNonProd*4 > maxIter ) nbNumPTB = std::max(1, nbNumPTB*2/3);
	}
