		...
		else
		{
			...
			for (inputFile >> content; content != "EOF"; inputFile >> content)
			{
				...
				else if (content == "MUST_DISPATCH_SECTION")
				{
					...
					{
						int clientNr = 0;
						inputFile >> clientNr >> cli[i].mustDispatch;

						// Check if the clients are in order
						if (clientNr != i + 1)
						{
							throw std::string("Clients are not in order in the list of mustDispatches");
						}
					}
				}
				else if (content == "PRIORITY_SECTION")
				{
					for (int i = 0; i <= nbClients; i++)
					{
						int clientNr = 0;
						inputFile >> clientNr >> cli[i].priority;

						// Check if the clients are in order
						if (clientNr != i + 1)
						{
							throw std::string("Clients are not in order in the list of priorities");
						}
					}
				}
