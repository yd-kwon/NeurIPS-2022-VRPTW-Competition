...
void Individual::evaluateCompleteCost()
{
	...
	for (int r = 0; r < params->nbVehicles; r++)
	{
		if (!chromR[r].empty())
		{
			...
			double prioritySum = params->cli[chromR[r][0]].priority;
            ...
            bool hasMustDispatch = static_cast<bool>(params->cli[chromR[r][0]].mustDispatch);
			for (int i = 1; i < static_cast<int>(chromR[r].size()); i++)
			{
                hasMustDispatch |= static_cast<bool>(params->cli[chromR[r][i]].mustDispatch);
				...
				prioritySum += params->cli[chromR[r][i]].priority;
            ...
            int capacityExcess = std::max(0, load - params->vehicleCapacity);
            int nbRoutes = 1;
            int nbRoutesMustDispatch = static_cast<int>(hasMustDispatch);
            int nbCustom = static_cast<int>(chromR[r].size());

            myCostSol.update(distance, prioritySum, nbCustom, capacityExcess, load, timeWarp, waitTime, nbRoutes, nbRoutesMustDispatch,
                             params->vehicleCapacity, params->config.capacityIdleThreshold, hasMustDispatch);
		}
	}
	...
	double penaltyCapacityIdle = params->penaltyCapacity * params->config.penaltyCapacityReduction;
	myCostSol.penalizedCost = myCostSol.calculatePenalizedCost(params->nbClients, params->config.dispatchNumLim, params->config.penaltyNumCustom, params->config.penaltyTheRatio, params->penaltyCapacity, penaltyCapacityIdle, params->penaltyTimeWarp, params->penaltyWaitTime);
...
void Individual::exportCVRPLibFormat(std::string fileName)
{
	...
	if (myfile.is_open())
	{
		...
		myfile << "Cost " << myCostSol.getTrueCost() << std::endl;
...
void Individual::printCVRPLibFormat()
{
	...
	std::cout << "Cost " << myCostSol.getTrueCost() << std::endl;
	std::cout << "Time " << params->getTimeElapsedSeconds() << std::endl;

    std::cout << "----- DATA" << std::endl;
    std::cout << "capacityIdleThreshold " << params->config.capacityIdleThreshold << std::endl;
    std::cout << "penalizedCost " << myCostSol.penalizedCost << std::endl;
    std::cout << "nbRoutes " << myCostSol.nbRoutes << std::endl;
    std::cout << "distance " << myCostSol.distance << std::endl;
    std::cout << "capacityExcess " << myCostSol.capacityExcess << std::endl;
    std::cout << "capacityIdle " << myCostSol.capacityIdle << std::endl;
    std::cout << "waitTime " << myCostSol.waitTime << std::endl;
    std::cout << "timeWarp " << myCostSol.timeWarp << std::endl;
    std::cout << "load " << myCostSol.load << std::endl;
    std::cout << "nbRoutesMustDispatch " << myCostSol.nbRoutesMustDispatch << std::endl;
    std::cout << "distanceMustDispatch " << myCostSol.distanceMustDispatch << std::endl;
    std::cout << "loadMustDispatch " << myCostSol.loadMustDispatch << std::endl;
